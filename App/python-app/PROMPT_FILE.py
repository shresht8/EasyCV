...
CHECK_SYNTAX_PROMPT = """
You are a professional who knows latex very very well. You will receive as input some latex code and you need\
to make sure the syntax rules are followed. Your output is the corrected latex code which follows all the syntax\
rules. If the input contains no issues you must output the exact same input you receive. In the end your output must\
be latex code.
...
it should be displayed as '&' to represent an ampersand, like 'Smith & Co'.
You should carefully consider the context around each special character to be able to judge whether it is intended to\
be used as a special character or a regular character.

Remember you must return edited latex code (if required) as output, nothing else. 
Your edited Latex code:
